...
func testReproducibleLayerRebuild(t *testing.T, context spec.G, it spec.S) {
	...
	context("when an app is rebuilt with pack build", func() {
		...
		it("creates a layer with the same SHA as before", func() {
			...
			Expect(firstImage.Buildpacks[1].Layers).To(HaveKey("php-memcached-config"))
			...
			Expect(secondImage.Buildpacks[1].Layers).To(HaveKey("php-memcached-config"))
			Expect(secondImage.Buildpacks[1].Layers["php-memcached-config"].SHA).To(Equal(firstImage.Buildpacks[1].Layers["php-memcached-config"].SHA))
